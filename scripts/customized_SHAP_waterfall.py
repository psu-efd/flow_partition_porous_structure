...
def custom_waterfall(shap_values, max_display=10, show=True):
    ...
    plt.yticks(ytick_pos, yticklabels[:-1] + [label.split('=')[-1] for label in yticklabels[:-1]], fontsize=18)
    ...
    custom_prediction_text = "\n"+r"$E[\alpha(Fr, \beta, C_d)] = $" + f" {base_values:.2f}"
    ax2.set_xticklabels([custom_prediction_text, ""], fontsize=16, ha="left")
    ...
    #ax3.set_xticklabels([r"$\alpha(Fr, \beta, C_d)$", "$ = "+format_value(fx, "%0.03f")+"$"], fontsize=12, ha="left")
    custom_prediction_text = r"$\alpha(Fr, \beta, C_d) = $" + f" {fx:.2f}"
    ax3.set_xticklabels([custom_prediction_text, " "], fontsize=16,
                        ha="center")
...
def waterfall_legacy(expected_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True):
    ...
    plt.axvline(expected_value, 0, 1, color="#bbbbbb", linestyle="--", linewidth=1.0, zorder=100)
    fx = expected_value + shap_values.sum()
    plt.axvline(fx, 0, 1, color="#bbbbbb", linestyle="--", linewidth=1.0, zorder=-1)
    ...
    ax2 = ax.twiny()
    ax2.set_xlim(xmin, xmax)
    ax2.set_xticks([expected_value, expected_value+1e-8],color="red")  # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
    ax2.tick_params(axis='x', length=20, color='red', width=2)
    ax2.set_xticklabels(["\n$E[f(X)]$", "\n$ = "+format_value(expected_value, "%0.03f")+"$"], fontsize=12, ha="center")
    ax2.spines['right'].set_visible(False)
    ax2.spines['top'].set_visible(False)
    ax2.spines['left'].set_visible(False)
