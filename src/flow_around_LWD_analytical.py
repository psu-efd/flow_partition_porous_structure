...
def plot_alpha_random_forest_regression_vs_truth(alpha_random_forest, alpha_truth, rmse, r2_score, Fr, beta, Cd):
    ...
    ax.text(0.05, 0.75, f"$r^2$ = {r2_score:.3f}", fontsize=14, color="black", ha="left")
    ...
    if bPlot_alpha_SHAP_values_for_features:
        ...
        for iPlot in range(3):
            ...
            if iPlot == 0:  #Fr colored with beta
                ...
                cbar = plt.colorbar(scatter_plot, shrink=1.0, fraction=0.03, pad=0.04, aspect=40)
                # Customize colorbar
                cbar.set_label(r"$\beta$", fontsize=16)  # Set colorbar title font size
                cbar.ax.tick_params(labelsize=14)  # Set colorbar label number font size
            ...
            ax.tick_params(axis='both', which='major', labelsize=14)
            ax.set_xlabel(features_to_plot[iPlot], fontsize=16)
            ax.set_ylabel("SHAP value for "+ features_to_plot[iPlot], fontsize=16)
            ...
            plt.savefig("alpha_dependence_plot_SHAP_SRH_2D_"+feature_names[iPlot]+".png", dpi=300, bbox_inches='tight', pad_inches=0.05)
...
def compare_simple_model_with_SRH_2D():
    ...
    with np.load('Fr_beta_C_A_h2prime_h2_alpha_arrays_SRH_2D.npz') as data:
...
def plot_alpha_results():
    ...
    for iFrs in range(len(Frs_to_show)):
        ...
        ax.set_title(r"$Fr$ = " + f'{Fr_to_show:.2f}', fontsize=16)
...
def solve_for_flume_experiment_cases():
    ...
    for idx, case_ID in enumerate(case_IDs):
        ...
        solution, bChoked, bConverged = solve_LWD_for_given_Fr_beta_C_A(Frs[idx], betas[idx], Cds[idx], True)
